...
import { Page, Container, PageHeading, Flex, Box } from 'components/Layout';

const TeamPage = props => {
  const team = props.data.allTeamJson.edges;
  const imageSizesByName = props.data.allFile.edges.reduce(
    (o, { node }) => ((o[node.name] = node.childImageSharp.fluid), o),
    {}
  );
          ...
          {team.map(({ node: person }) => (
            ...
            >
              <Img
                ...
                fluid={imageSizesByName[person.name.toLowerCase()]}
